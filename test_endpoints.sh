...
YOUR_ACCESS_TOKEN=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJuZXd1c2VyMyIsImV4cCI6MTcyNDA4NjQyNH0.P062ax0KVPMb2AJMDu7yPejWtzFcvXqBbZuo84hnUnA
...
curl -X POST http://localhost:7600/chat-sessions/1/messages/ \
  -H "Authorization: Bearer $YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"content": "Hello, AI!", "message_type": "user"}'
...
curl -X POST http://localhost:7600/api/bot-responses/ \
  -H "Content-Type: application/json" \
  -d '{"message_id": 1, "content": "Hello, human!", "model_version": "v1.0", "tokens_used": 10}'
