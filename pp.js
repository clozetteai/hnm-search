...
const AICompanySearch = () => {
  ...
  const fetchProducts = useCallback(async (message = '', newPage = 1) => {
    setIsLoading(true);
    try {
      const response = message
        ? await apiClient.search(message, newPage)
  ...
  const handleSearch = useCallback(async (message) => {
    setIsLoading(true);
    setCurrentMessage(message);
    setPage(1);
    setSearchResults([]);
    try {
      const response = await apiClient.search(message);
      if (response.products) {
  ...
  useEffect(() => {
    if (page > 1) {
      fetchProducts(currentMessage, page);
    }
  }, [page, currentMessage, fetchProducts]);
